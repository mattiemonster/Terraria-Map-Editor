...
{
    public class RemoveAllHoneyPlugin : BasePlugin
    {
        public RemoveAllHoneyPlugin(WorldViewModel worldViewModel)
            : base(worldViewModel)
        {
            Name = "Remove All Honey";
        }
        ...
        public override void Execute()
        {
            ...
            if (MessageBox.Show("Are you sure you wish to remove all honey from this world?", "Delete Honey?",
            ...
            for (int x = 0; x < _wvm.CurrentWorld.TilesWide; x++)
            {
                for (int y = 0; y < _wvm.CurrentWorld.TilesHigh; y++)
                {
                    ...
                    if (_wvm.CurrentWorld.Tiles[x, y].LiquidType == LiquidType.Honey)
                    {
